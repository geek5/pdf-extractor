...
const AnnotationLayer = require('../../pdfjs-utils/annotation_layer');
const { PDFLinkService } = require('../../pdfjs-utils/pdf_link_service');
				...
				return page.getAnnotations().then((annotationsData) => {
					...
					if (annotationsData.length) {
						...
						for (var i = 0; i < annotationsData.length; i++) {
							...
							var annotationContainer = AnnotationLayer.getHtmlElement(data, page, viewport, linkservice);
							items[i] = { el: annotationContainer.outerHTML || annotationContainer.innerHTML, data }
							// items[i]['el'] = annotationContainer.outerHTML || annotationContainer.innerHTML;
							// items[i]['data'] = data
						}

						return this.writeStringToFile(JSON.stringify(items), this.getPathForFile('page-' + page.pageNumber + '-annotation.json'));
					}
